...
def main():
    ...
    for v in m["videos"]:
        ...
        # 防路径逃逸:transcript 必须解析在 outdir 内(../ 之类按输入问题拒跑,同 manifest 校验 exit 2)
        if not tpath.resolve().is_relative_to(out.resolve()):
            print(f"视频 {no} 的 transcript 路径越出 outdir(拒读): {v['transcript']}", file=sys.stderr)
            return 2
