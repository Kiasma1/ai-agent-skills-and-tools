...
def page(*, tabs_html=None, cover=COVER, chapters=GOOD_CH, subpages=SUBPAGES, sub_entries=SUB_ENTRIES,
    ...
    return (f'<html {lang}><head><style>{TOKENS}{css}</style></head><body>'
            ...
            f'<section class="panel on" id="panel-glance">{glance}{srcnote}{forbidden_class}{extra_blocks}{extra}</section>'
...
# ================================================================ 占位残留门禁(契约模式专属:lint_distill 传 html 才查)
def test_placeholder_residue_flagged_in_contract_mode():
    # 成品页残留 (dummy)/(示例)/{书名}/{作者} -> 契约模式拦(Step6 须删干净 dummy)
    dirty = page(extra_blocks='<p>{书名} 由 {作者} 著(dummy)(示例)</p>')
    v = lint_distill(distill(), html=dirty)
    assert any("占位残留" in x for x in v)


def test_placeholder_slug_and_moustache_flagged():
    # SLUG_PLACEHOLDER / {{占位}} 同样命中残留门禁
    v = lint_distill(distill(), html=page(extra_blocks='<a href="SLUG_PLACEHOLDER.html">{{title}}</a>'))
    assert any("占位残留" in x and "SLUG_PLACEHOLDER" in x for x in v)


def test_placeholder_not_checked_without_html():
    # 纯 JSON 契约(不传 html)不查占位;骨架 raw lint 也不查
    assert lint_distill(distill()) == []


def test_placeholder_clean_contract_page_passes():
    assert lint_distill(distill(), html=page()) == []


# ================================================================ Zero-Hex 扩展(属性区:内联 style / SVG fill 等)
def test_attr_hex_inline_svg_flagged():
    # 内联 SVG 属性区 fill="#ffffff" -> 属性区硬编码色,不豁免
    v = lint_html(page(extra_blocks='<svg viewBox="0 0 10 10"><rect fill="#ffffff" width="10" height="10"/></svg>'))
    assert any("属性区硬编码" in x for x in v)


def test_attr_hex_inline_style_flagged():
    v = lint_html(page(extra_blocks='<p style="color:#ff0000">红字</p>'))
    assert any("属性区硬编码" in x for x in v)


def test_attr_hex_script_exempt():
    # <script> 内 hex 是 JS 颜色兜底,设计豁免(整块剥离后再扫)
    assert lint_html(page(extra_blocks='<script>var y = "#abc123";</script>')) == []


def test_attr_hex_data_uri_exempt():
    # data: URI(dummy 封面 = URL 编码 SVG)内的编码色不触发属性区扫描
    uri = "data:image/svg+xml,%3Csvg%20fill='%23ffffff'/%3E"
    assert lint_html(page(extra_blocks=f'<img src="{uri}" alt="x">')) == []


# ================================================================ reviews 双向降级校验(enrich 一致性)
def test_enrich_reviews_present_but_block_absent_flagged():
    # reviews 非 null 但页面缺 .reviews 块 -> 拦(反向;正向 null-仍在 已有测试)
    enrich = {"author_page": {"name": "x"}, "views_page": {"topics": []}, "reviews": {"items": []}}
    v = lint_enrich_consistency(page(drop_class="reviews"), enrich)
    assert any("reviews" in x and "缺" in x for x in v)


# ================================================================ .subpage-from 归属语境(有 .subpage-bar 必含)
def test_subpage_bar_without_from_flagged():
    subs = ('<section class="subpage" id="sub-author" hidden>'
            '<div class="subpage-bar"><button class="sub-back" data-sub-back>返回</button>'
            '<h2 class="subpage-title">作者</h2></div>'
            f'{AUTHOR_BODY}</section>'
            '<section class="subpage" id="sub-views" hidden><button class="sub-back" data-sub-back>返回</button></section>')
    v = lint_html(page(subpages=subs))
    assert any("subpage-from" in x for x in v)


def test_subpage_bar_with_from_ok():
    subs = ('<section class="subpage" id="sub-author" hidden>'
            '<div class="subpage-bar"><button class="sub-back" data-sub-back>返回</button>'
            '<span class="subpage-from">《书》· 延伸档案</span></div>'
            f'{AUTHOR_BODY}</section>'
            '<section class="subpage" id="sub-views" hidden><button class="sub-back" data-sub-back>返回</button></section>')
    assert lint_html(page(subpages=subs)) == []


# ================================================================ 输入错误 exit 2(页面/JSON 缺失损坏;exit 1 严格留给内容违规)
def test_main_missing_page_returns_2(tmp_path, monkeypatch, capsys):
    import verify_page
    monkeypatch.setattr(sys, "argv", ["verify_page.py", str(tmp_path / "nope.html")])
    assert verify_page.main() == 2
    assert "输入" in capsys.readouterr().err


def test_main_bad_distill_json_returns_2(tmp_path, monkeypatch, capsys):
    import verify_page
    bad = tmp_path / "distill.json"
    bad.write_text("{not json,,}", encoding="utf-8")
    pg = tmp_path / "page.html"
    pg.write_text(page(), encoding="utf-8")
    monkeypatch.setattr(sys, "argv",
                        ["verify_page.py", str(pg), "--distill", str(bad), "--skip-interact"])
    assert verify_page.main() == 2
    assert "输入" in capsys.readouterr().err


def test_main_clean_page_returns_0(tmp_path, monkeypatch):
    import verify_page
    pg = tmp_path / "page.html"
    pg.write_text(page(), encoding="utf-8")
    monkeypatch.setattr(sys, "argv", ["verify_page.py", str(pg), "--skip-interact"])
