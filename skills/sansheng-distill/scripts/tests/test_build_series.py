...
def test_transcript_path_escape_rejected(tmp_path):
    # transcript 用 ../ 越出 outdir:必须走输入校验拒跑(exit 2),不得读取越界文件
    outside = tmp_path / "escape.txt"
    outside.write_text("越界内容。" * 100, encoding="utf-8")
    outdir = tmp_path / "out"
    videos = [{"no": 1, "title": "第1集", "url": "https://www.youtube.com/watch?v=x1",
               "transcript": "../escape.txt"}]
    m = {"slug": "test-series", "series_title": "测试系列", "author": "测试UP",
         "platform": "youtube", "videos": videos}
    p = tmp_path / "series-input.json"
    p.write_text(json.dumps(m, ensure_ascii=False), encoding="utf-8")
    r = run(p, outdir)
    assert r.returncode == 2 and "越出" in r.stderr
    assert not (outdir / "book.txt").exists()
