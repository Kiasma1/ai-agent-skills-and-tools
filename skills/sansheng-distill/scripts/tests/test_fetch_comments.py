...
# --- yt-dlp 子进程超时(180s):按单视频失败隔离,不挂死整批 ---
def test_youtube_timeout_isolated(monkeypatch):
    def boom(*a, **k):
    ...
    videos = [{"no": 1, "url": "https://www.youtube.com/watch?v=a"},
              {"no": 2, "url": "https://www.youtube.com/watch?v=b"}]
    out = fc.collect_comments(videos, "youtube", 30)
    assert out == [[], []]  # 超时 = 单视频失败,不整体崩


def test_main_timeout_all_fail_exit2(monkeypatch, tmp_path):
    def boom(*a, **k):
        raise subprocess.TimeoutExpired(cmd="yt-dlp", timeout=180)
    monkeypatch.setattr(fc.subprocess, "run", boom)
    series = {"slug": "s", "series_title": "t", "author": "u", "platform": "youtube",
              "videos": [{"no": 1, "title": "x", "url": "https://www.youtube.com/watch?v=a"}]}
    p = tmp_path / "series.json"
    p.write_text(json.dumps(series, ensure_ascii=False), encoding="utf-8")
    out = tmp_path / "comments.json"
    rc = fc.main(["--series", str(p), "--out", str(out)])
    assert rc == fc.decide_exit(0, "youtube") == 2  # 全失败 → decide_exit 契约
