...
def test_in_batch_duplicate_new_concept_rejected(tmp_path):
    # 同批两条同名 NEW_CONCEPT:索引里都没有,但批内重复会建出两个同名节点,必须拦
    idx = tmp_path / "idx.json"; mg = tmp_path / "m.json"
    w(mg, [{"concept": "复利", "relation": "NEW_CONCEPT", "one_liner": "", "entry": entry(slug="b1")},
           {"concept": "复利", "relation": "NEW_CONCEPT", "one_liner": "", "entry": entry(slug="b2")}])
    r = run("register", "--index", str(idx), "--merge", str(mg))
    assert r.returncode == 1 and "重复" in r.stderr

def test_in_batch_chain_new_then_supports_passes(tmp_path):
    # 批内链:NEW_CONCEPT 先建节点,同批 SUPPORTS 指向它应放行(dry-run exit 0)
    idx = tmp_path / "idx.json"; mg = tmp_path / "m.json"
    w(mg, [{"concept": "安全边际", "relation": "NEW_CONCEPT", "one_liner": "", "entry": entry(slug="b1")},
           {"concept": "安全边际", "relation": "SUPPORTS", "entry": entry(slug="b2")}])
    r = run("register", "--index", str(idx), "--merge", str(mg), "--dry-run")
    assert r.returncode == 0, r.stderr
    # 实跑同样应成:两条 entry 挂同一个概念节点
    r2 = run("register", "--index", str(idx), "--merge", str(mg))
    assert r2.returncode == 0, r2.stderr
    data = json.loads(idx.read_text(encoding="utf-8"))
    assert len(data["concepts"]) == 1 and len(data["concepts"][0]["entries"]) == 2

def test_corrupt_merge_json_exit1_no_traceback(tmp_path):
    idx = tmp_path / "idx.json"; mg = tmp_path / "m.json"
    mg.write_text("{ 不是合法 JSON", encoding="utf-8")
    r = run("register", "--index", str(idx), "--merge", str(mg))
    assert r.returncode == 1 and "Traceback" not in r.stderr

def test_missing_merge_file_exit1_no_traceback(tmp_path):
    idx = tmp_path / "idx.json"
    r = run("register", "--index", str(idx), "--merge", str(tmp_path / "nope.json"))
    assert r.returncode == 1 and "Traceback" not in r.stderr

def test_merge_not_array_exit1(tmp_path):
    idx = tmp_path / "idx.json"; mg = tmp_path / "m.json"
    w(mg, {"concept": "X"})  # 合法 JSON 但不是数组
    r = run("register", "--index", str(idx), "--merge", str(mg))
    assert r.returncode == 1 and "数组" in r.stderr

def test_corrupt_index_exit1_no_traceback(tmp_path):
    idx = tmp_path / "idx.json"; mg = tmp_path / "m.json"
    idx.write_text("{ 损坏", encoding="utf-8")
    w(mg, [{"concept": "X", "relation": "NEW_CONCEPT", "one_liner": "", "entry": entry()}])
    r = run("register", "--index", str(idx), "--merge", str(mg))
    assert r.returncode == 1 and "Traceback" not in r.stderr
