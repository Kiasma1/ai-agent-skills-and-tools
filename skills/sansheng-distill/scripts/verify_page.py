...
# 非贪婪 (.*?)</section> 假定无嵌套 <section>(骨架保证子视图/章节内不再套 section)
...
# 模板占位残留(仅契约模式 = 传 --distill 时查;骨架 raw lint 不查,Step6 须删干净 dummy)
TEMPLATE_RESIDUE = ("(dummy)", "(示例)", "(示例)", "SLUG_PLACEHOLDER", "{书名}", "{作者}", "{{")
...
def lint_html(html: str, distill: dict | None = None, enrich: dict | None = None) -> list:
    ...
    # 注:非贪婪 (.*?)</section> 假定章内无嵌套 <section>(骨架保证;同 SUBPAGE_SECTION_RE)
    ...
    for sid, body in present_subs.items():
        ...
        # 归属语境(html-spec §3 step8):有 .subpage-bar 则必含 .subpage-from
        if _has_class(body, "subpage-bar") and not _has_class(body, "subpage-from"):
            v.append(f"[lint] 子视图 #{sid} 的 .subpage-bar 缺 .subpage-from 归属语境(html-spec §3 step8)")
    ...
    if "src-note" not in re.sub(r"<!--.*?-->", "", html, flags=re.S):
    ...
    # Zero-Hex 扩展(文档其余部分):剥 <style>(上面已单扫)/ <script>(JS 颜色兜底,设计豁免)/
    #   data: URI(dummy 封面=URL 编码 SVG,不触发)后,属性区(内联 style=/SVG fill=/stroke= 等)同样禁字面 hex。
    #   逐标签扫属性(<[^>]*> 先行抽取),防在大文本段上 [\w-]+ 灾难性回溯。
    rest = re.sub(r"<style\b[^>]*>.*?</style>", "", html, flags=re.S)
    rest = re.sub(r"<script\b[^>]*>.*?</script>", "", rest, flags=re.S)
    rest = re.sub(r'data:[^"\')\s]+', "", rest)
    attr_hex = [(am.group(1), am.group(2))
                for tm in re.finditer(r"<[^>]*>", rest)
                for am in re.finditer(r'([\w-]+)\s*=\s*"([^"]*)"', tm.group(0))
                for _hm in HEX_RE.finditer(am.group(2))]
    if attr_hex:
        ctx = "; ".join(f'{a}="{val[:40]}"' for a, val in attr_hex[:5])
        v.append(f"[lint] 属性区硬编码色 {len(attr_hex)} 处: {ctx}(token 块外零 hex,内联 style/SVG 属性不豁免)")
    # lang
...
def lint_enrich_consistency(html: str, enrich: dict) -> list:
    ...
    # reviews 双向:null → .reviews 块删;非 null → .reviews 块必在(§1.5,对齐 author_page/views_page)
    rv = enrich.get("reviews")
    if rv is None and _has_class(html, "reviews"):
        v.append("[lint] reviews 为 null 但 .reviews 块仍在(应整块删,§1.5)")
    if rv is not None and not _has_class(html, "reviews"):
        v.append("[lint] reviews 非 null 但缺 .reviews 块(§1.5)")
    return v
...
def lint_placeholder_residue(html: str) -> list:
    """模板占位残留门禁(仅契约模式 = 传 --distill 时;骨架本身全是 dummy,raw lint 不查):
    成品页须把 (dummy)/(示例)/{{占位}}/SLUG_PLACEHOLDER/{书名}/{作者} 删干净(Step6)。"""
    hits = [(p, html.count(p)) for p in TEMPLATE_RESIDUE if html.count(p)]
    if hits:
        detail = ", ".join(f"{p}×{n}" for p, n in hits)
        return [f"[lint] 模板占位残留: {detail} — Step6 须删干净 dummy"]
    return []


def lint_distill(data: dict, html: str | None = None) -> list:
    """distill.json 契约门禁(§7.5 可机拦部分 G8-G21 + G4 扩展):narrative(G9)/ 章标题(G8)/ §5.1 六类 anchor /
    ...
    传 html 时追加模板占位残留门禁(Step6 须删干净 dummy)。"""
    v = []
    ...
    if not (cq and str(cq).strip()):
        ...
        if not re.search(r"[?？]\s*$", cq_s):
            v.append("[distill] core_question 非疑问句(须以 ? 或 ？ 结尾)(G19)")
        one_liner = ((data.get("napkin") or {}).get("one_liner")) or ""
    ...
    # 模板占位残留(契约模式专属;须传 html 才查,骨架 raw lint 不查)
    if html is not None:
        v += lint_placeholder_residue(html)
    return v
...
def author_smoke(path: Path, screenshot: str | None) -> list:
    ...
    try:
        ...
        print("[输入] 需 pip install playwright && playwright install chromium", file=sys.stderr)
        raise SystemExit(2)
    v, errors = [], []
    with sync_playwright() as p:
        try:
            ...
            if "playwright install" in str(e) or "Executable" in str(e):
                print("[输入] chromium 未安装,需 playwright install chromium", file=sys.stderr)
                raise SystemExit(2)
            raise
...
def smoke(path: Path, screenshot: str | None) -> list:
    ...
    with sync_playwright() as p:
        ...
        # 三级类别必须可读:emoji 只作辅助编码，节点内显示短类别词，画布上同时给完整图例。
        category_readability = pg.evaluate("""() => {
            const dataEl = document.getElementById('bd-mindmap-data');
            let rawText = '';
            try { rawText = JSON.stringify(JSON.parse(dataEl ? dataEl.textContent : '{}')); } catch (_) {}
            const rendered = Array.from(document.querySelectorAll('#bd-mindmap svg text')).map(el => el.textContent || '').join('');
            const legend = (document.querySelector('#bd-mindmap .mm-legend') || {}).textContent || '';
            return [
                { icon: '🧠', required: rawText.includes('🧠'), node: rendered.includes('🧠 原理｜'), legend: legend.includes('🧠 原理/模型') },
                { icon: '💡', required: rawText.includes('💡'), node: rendered.includes('💡 概念｜'), legend: legend.includes('💡 核心概念') },
                { icon: '⚖️', required: rawText.includes('⚖️'), node: rendered.includes('⚖️ 规则｜'), legend: legend.includes('⚖️ 行动规则') }
            ];
        }""")
        for item in category_readability:
            if item["required"] and not item["node"]:
                v.append(f"[渲染] 脑图 {item['icon']} 类节点仅靠 emoji 表意，缺少可见类别文字")
            if item["required"] and not item["legend"]:
                v.append(f"[渲染] 脑图 {item['icon']} 类节点缺少画布图例说明")
...
def main():
    ...
    ap.add_argument("--distill", help="distill.json 路径:传入则追加契约门禁(G8-G21 + G4 扩展)+ 渲染侧查重")
    ...
    a = ap.parse_args()
    # 输入问题(页面/JSON 缺失或损坏)→ stderr + exit 2;exit 1 严格留给内容违规,0 = 全过
    try:
        html = Path(a.page).read_text(encoding="utf-8")
    except OSError as e:
        print(f"[输入] 页面文件缺失或不可读: {a.page}({e})", file=sys.stderr)
        return 2
    # 作者演变页(结构不同)走独立门禁,不套蒸馏页 REQUIRED_CLASSES
    if is_author_page(html, a.author_json):
        if a.author_json:
            try:
                author = json.loads(Path(a.author_json).read_text(encoding="utf-8"))
            except (OSError, ValueError) as e:
                print(f"[输入] --author-json 缺失或 JSON 损坏: {a.author_json}({e})", file=sys.stderr)
                return 2
            perr = []
        else:
            author, perr = _load_author_json(html, None)
    ...
    try:
        ...
        print(f"[输入] --distill/--enrich JSON 缺失或损坏: {e}", file=sys.stderr)
        return 2
