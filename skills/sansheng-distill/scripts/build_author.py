...
def collect_distills(args):
    ...
    paths = []
    cached = {}  # path -> 已解析 dict(author 过滤趟已读过,缓存复用免二次读盘)
    if args.inputs:
        paths = [Path(p) for p in sorted(globmod.glob(args.inputs))]
        if not paths:
            # glob 一个文件都没匹配到 = 输入本身有误(多半是拼写/路径错),按输入问题 exit 2
            print(f"--inputs 未匹配到任何文件: {args.inputs}(检查 glob 拼写/路径)", file=sys.stderr)
            sys.exit(2)
        ...
        for p in sorted(Path(args.data_root).glob("*/distill.json")):
            try:
                ...
                continue  # 第一趟过滤静默跳过坏文件(扫描分组不该被单本坏档拖垮)
    ...
    for p in paths:
        if p in cached:
            distills.append(cached[p])  # author 模式命中缓存,不重复读盘
            continue
        try:
...
def derive_turns(books, manual, enrich):
    """§4.3:相邻年共享概念 stance 语义对立 -> 候选(与 enrich 概念互斥,故 verdict 恒 null);合并 enrich.shifts。"""
        ...
        for name in ca:
            if name in cb and name not in cand_seen and name not in enrich_concepts:
                ...
                if differ and opposed:
                    cand_seen.add(name)
                    # verdict/external 恒 None/[]:循环条件已排除 enrich_concepts,
                    # 而 shift_by_concept 的键恰好就是 enrich 概念,查之必空,故直接写字面量
                    raw.append({
...
def build_author_json(distills, manual, enrich, index_path=None):
    ...
    if not books:
        # 门槛未达的另一形态(契约 exit 3):>=2 部已蒸但全部缺 pub_year,
        # 时间线为空,下游派生(earliest=books[0])无可依托,明确拒跑而非裸栈
        print(f"{len(distills)} 部已蒸但全部缺 pub_year,无法排思想演变时间线;"
              "请在各书 distill.json 补 pub_year 后重跑", file=sys.stderr)
        sys.exit(3)
    warnings = []
    for slug in missing:
...
def main():
    ...
    # manual/enrich 是人工输入文件:缺失/损坏属输入问题(exit 2),不得裸栈
    manual = {}
    if manual_present:
        try:
            manual = load_json(manual_path)
        except (json.JSONDecodeError, OSError) as e:
            print(f"manual 文件读取/解析失败({manual_path}): {e}", file=sys.stderr)
            return 2
    enrich = {}
    if a.enrich and Path(a.enrich).is_file():
        try:
            ...
            print(f"enrich 文件读取/解析失败({a.enrich}): {e}", file=sys.stderr)
            return 2
