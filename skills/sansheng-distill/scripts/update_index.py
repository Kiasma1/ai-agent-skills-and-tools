...
def load_index(path: Path) -> dict:
    ...
    try:
        ...
        # 索引损坏属输入/校验失败类(exit 1),给清楚报错而非裸栈
        print(f"索引文件读取/解析失败({path}): {e}", file=sys.stderr)
        sys.exit(1)

def validate(index: dict, items: list) -> list:
    errors, existing = [], {c["concept"] for c in index["concepts"]}
    batch_seen = set()  # 本批已登记的概念:批内重复 NEW_CONCEPT 判重 + 批内链式引用放行
    for i, it in enumerate(items):
        ...
        elif rel == "NEW_CONCEPT" and name in batch_seen:
            errors.append(f"[{i}] {name}: 本批内重复 NEW_CONCEPT 同名概念(会同名建两个概念节点)")
        if rel in RELATIONS - {"NEW_CONCEPT"} and name not in existing and name not in batch_seen:
            errors.append(f"[{i}] {name}: 标 {rel} 但索引没有该概念")
        if rel == "NEW_CONCEPT" and name:
            batch_seen.add(name)  # 登记后本批后续条目可链式引用(SUPPORTS/REFINES/...)
    return errors
...
def main():
    ...
    try:
        ...
        # merge 文件缺失/损坏属输入失败(exit 1),不裸栈
        print(f"merge 文件读取/解析失败({a.merge}): {e}", file=sys.stderr); return 1
    if not isinstance(items, list):
        print(f"merge 文件必须是 JSON 数组: {a.merge}", file=sys.stderr); return 1
    ...
    if path.exists():
        # .bak 有意只留单代:防误操作即可恢复;历史版本交给 git
