...
| **v4 深度分析字段**(核心问题 / 论证链阶梯 / 观点挂柱 / 跨域命名 / 公式读法 / 因果草图 / 章钩子) | 本 skill v4(深度分析批次 B/B-2) | `core_question` / `arguments.chain_steps` / `core_ideas[].pillar` / `cross_domain[]` / `napkin.formula_read` / `napkin.sketch` / `chapters[].hook` | §3 餐巾纸四件套 + §4.5.1/§4.5.12-4.5.15(hook 属 Pass 2) |

执行顺序(两遍):
1. **Pass 1**:§3(三轮,含餐巾纸四件套 formula_read/sketch)→ §4(四嫁接件)→ §4.5(延展字段,含 v3 cover_intro / credibility_verdict / chain_step 归站 + v4 core_question / cross_domain / chain_steps / pillar)→ §5(逐条补 anchor + evidence_level)。
2. **Pass 2**:§3.5(逐章详实转述 + `action_chain[].detail` 扩写,长书 subagent fan-out)。
3. §7 自查门禁(G1-G21,+G4 扩展 a/b;不过则回补)。超长书在 Pass 1 上套 §8 分组外壳。

**`quiz` 字段停产(2026-07-03)**:页面改浏览型、删自测复习板块;§9 已弃用,新蒸不产 `quiz`,§7 门禁无 quiz 项(见 §9)。
...
- **破折号统一(v4 批次C,Q7-12,机检)**:**转述文本(narrative / summary / 卡片文案 / 编辑点评等一切自己的话)破折号一律用 `--`(两个半角连字符),禁全角 `——` / `—`**;**只有 `excerpts` / `quotes` 的原文照录字段保留原样不改**(项目 Agent 指令中的「原文照录除外」)。verify 机拦:页面可见转述正文出现全角 `——`/`—` 即打回(blockquote / qw-card 原文区豁免)。
...
1. **分组**:按章切组,**每组 ≤5 章**;当前 Agent 支持任务委派时每组派 1 个高推理能力 worker,各拿「本组各章 Pass 1 骨架 + 本组原文切片」;不支持时由主 Agent 按组串行执行。
2. **组内串行**:worker 或主 Agent 在组内逐章写 narrative + excerpts(串行,保上下文连贯),不跳章。
...
## 4.6 脑图拆分(① `.bd-mindmap-wrap`,自绘 SVG 知识树:关键词路标 + 判断句副文本,先读骨架再读血肉)
...
> **`quiz` 字段已停产(2026-07-03)**:页面改浏览型、删自测复习板块,新蒸的 distill.json **不再生成 `quiz[]`**(见 §9)。存量 distill.json 里残留的 `quiz` 无害,填页时忽略即可,不用回改。
...
> **本节已弃用,不再执行。** 页面于 2026-07-03 从「学习型(含自测)」改为「浏览型(纯呈现核心内容)」,删除自测复习板块整套,骨架已无 `panel-quiz` / `.quiz-card` / ts-fsrs 复习卡。因此:
> - **蒸馏时不再生成 `quiz[]`**(§6 schema 的 `quiz` 字段停产)。
> - `core_ideas` 原本兼作自测出题来源,现只落头部与①②的呈现(见 html-spec §1.2.1),不再喂自测。
> - 存量 distill.json 里的 `quiz` 数据无害,保留不动、填页时忽略。
> - **§7 门禁无 quiz 项**:现门禁为 G1-G21 + G4 扩展(v2 的 G1-G15 + v3 的 G16-G18 + v4 的 G19-G21 与 G4 扩展 a/b),均不含任何自测 / quiz 门槛。
...
|---|---|---|---|
| **video-to-subtitle-summary**(可选 Skill,从当前 Agent 的 Skill 注册表定位) | 出字幕/转写文本(yt-dlp 抓字幕 / faster-whisper ASR / AI Douyin 下载非 YT) | ✅ 抓字幕 | **默认**:口播/知识型,转写就是价值 |
| **Gemini 视频分析工具**(可选,如 `sansheng-gemini-video`) | Gemini 原生看画面+音频 -> 转写/结构化理解 | ✅ `fileData.fileUri` 在线看 | **无字幕 / 需画面语义**(演示/代码/图表/PPT) |

**取材 cascade(按平台分流,能不下载就不下载):**

▎**YouTube**(在线优先)
1. 抓字幕:若当前 Agent 已安装 `video-to-subtitle-summary`,从其注册路径运行 `scripts/download_youtube_subtitles.py <url> --output-dir raw/{NN}_{id} --languages <manuallang>,en-US,en,zh-Hans`;否则用 `yt-dlp` 抓字幕并按下述规则清洗。
...
4. 无字幕 OR 需画面语义 -> 从当前 Agent 的 Skill/工具注册表定位兼容的 Gemini 视频分析工具,运行其 `analyze_video.py <url> --prompt "逐字转写带[MM:SS],逐字照录不总结" --fps 0.2 --start/--end`(低 fps + 裁片段控成本;实测 ~$0.01/2min,40min ≈ $0.2;转写质量实测 ≈ 人工字幕)。
5. 兜底 -> 下载音频 + faster-whisper(`ASR_BACKEND=faster-whisper`)。

▎**非 YouTube**(B站/抖音/小红书:**先下载再解析**)
- `video-to-subtitle-summary`:AI Douyin 代理拿直链下载 -> 有平台字幕抓字幕,否则 faster-whisper ASR。需画面语义 -> 下载后**本地文件 inline** 喂当前可用的 Gemini 视频分析工具(非 URL)。

▎**本地视频/音频文件**
- faster-whisper(纯语音)/ inline 喂当前可用的 Gemini 视频分析工具(需画面)。

**热度元数据(供 html-spec §V.3 热度条 / Q6-3)**:逐条 `yt-dlp --skip-download --print "%(view_count)s|%(like_count)s|%(comment_count)s|%(upload_date)s|%(channel_follower_count)s"`。⚠️ `--flat-playlist` 拉列表时这些字段为 `NA`,**必须逐条 full extract**;取即时快照值 + 标 `as_of` 日期(播放量随时间变)。
...
- **v2/v3/v4 门禁(G8-G21 + G4 扩展)对视频照检**,仅 G9 narrative 走视频档 400、G14 excerpts 视频不检、G18 credibility_verdict 视频不检(可省)、G8 黑名单含 `视频\d+`(详见 §V.7);G10-G13 / G15 / G16 / G17 同书籍不降。
...
`chapters[]` 是页面脉络骨架,撑起①脑图一级节点与②手风琴章节,**目标 5-8 条**(与成书章节数量级一致)。按视频数量与单集时长两种切法:
...
- **v4 深度分析字段(§3 餐巾纸 + §4.5.12-4.5.15)**:视频**同书籍必产** -- `core_question`(G19 照检)/ `arguments.chain_steps`(G20 照检)/ `napkin.formula_read`(G4 扩展 a 照检)/ `core_ideas[].pillar` / `cross_domain[]`(宁缺毋滥)/ `chapters[].hook`(若产 ≤20 字,G21 照检);`napkin.sketch` 可降级(若产则 G4 扩展 b 照检)。
