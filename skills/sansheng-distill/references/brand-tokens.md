...
| `--line` | 边框 / 分隔线 | |
| `--red` | 强调·警示 / eyebrow / arg-restate 竖线 | |
| `--blue` | 强调·链接 / 规则关键词 | |
| `--green` | 强调·行动 / 正向(品牌绿族) | |
...
| `--surface` | 卡片底(半透明) | |
| `--surface-strong` | 强卡底(arg-restate / 弹层) | |
...
```css
body[data-theme="minimal"] .arg-restate{border-left-color:var(--ink)}
...
v4.1 起脑图为**自绘 SVG 深色版**(零第三方库,markmap / mind-elixir 均已移除),配色**与页面 7 主题解耦、不随换肤**:

- 画布恒深色(深墨绿):切主题时脑图不变色;`window.__bdMindmapRetheme()` 是 **no-op**(空函数,仅留调用位兼容旧调用)。
- 颜色走 `:root` 里一套专用 **`--mm-*` dark token**(`--mm-bg` / `--mm-ink` / `--mm-sec` / `--mm-judge` / `--mm-line` / `--mm-root-*` / `--mm-chip-*` / `--mm-accent` 等),与 §1 的 21 个页面 token 互不混用;组件 CSS 只用 `var(--mm-*)`,SVG 生成器经 `getComputedStyle` 读**同一套** `--mm-*`(单一来源),JS 内 hex 仅 fallback 兜底。
- 层级色彩语义:root = 稍亮墨绿圆角卡片(`--mm-root-*`)/ 一级分支 = 金奶油纯文字(`--mm-sec`)/ 二级 = 米白关键词(`--mm-ink`)+ 浅青判断句(`--mm-judge`)+ 章码 chip(`--mm-chip-*`)/ 引线 = 淡青灰(`--mm-line`)。
...
- `theme-picker` 放**右下**(`position:fixed; right/bottom`)-- 避开站壳右上 mini-player 区;新增 fixed 元素一律优先放底部。(v2 的 `.src-toggle` 出处开关已废,verify 拦,勿再加)
- 上站后 extract 按 `.theme-picker` 类名剔除浮层(站壳有自己的控件),所以浮动控件**必须用这个约定类名**。
- **主题切换器 = 触发按钮 + 竖列弹层(2026-07-03 改)**:`.theme-picker` 默认**收起**为一个小触发按钮 `.tp-trigger`(🎨 图标 + 文字「主题」+ 极简 SVG caret,与骨架 `.tp-ico` 一致);点它切 `.theme-picker[data-open="1"]` 弹出竖向单列菜单 `.tp-menu`(`flex-direction:column`,窄宽约 172px,右对齐、不遮左侧正文)。菜单内放 7 颗 `<button data-theme-pick="{主题名}">`;当前生效项由脚本写 `aria-current="true"` 高亮。点主题项应用并收起;点弹层外 / 按 Esc 收起。切主题只换 `body[data-theme]` 的 token 块;脑图为自绘 SVG 恒深色、**不随换肤**(`__bdMindmapRetheme` 为 no-op,§4)。
...
2. 主题组件微调规则(如 `body[data-theme="minimal"] .arg-restate{…}`,选择器后带后代)**不算 token 块**,内部禁 hex。
