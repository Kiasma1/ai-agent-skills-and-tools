...
### 1.2 区块规格表(5 板块内部区块;原设计「方案」为仓外文档、未随仓发布,逐块规格以本节为准)
...
| 3 | **全书脑图** `<div class="bd-mindmap-wrap">` | distill:`chapters[]` + `core_ideas` + `mental_models`/`concepts`/`decision_rules` + `enrich` | 位置**固定在因果骨架草图之下、核心观点之前**(F5)。`<script type="application/json" id="bd-mindmap-data">` 内**充实知识树 JSON**(单键 `nodeData` + 可缺省 `layout`,规范见 §5;`arrows`/`summaries` 已废,二级核心观点带 `hyperLink:'#ch-N'`,SVG 版**全展开不折叠**、靠 viewBox 缩放看全图)+ `<div id="bd-mindmap">`(空,脚本层渲染)+ `.mm-hint`=动作指令式说明「点任一分支,直达该章精读」;两个 id 勿改。点二级节点 → 切②、展开目标章、滚动、章头高亮 2 秒(章码传送门,§4.交互);全图只画父子直角引线,不画任何跨枝关联箭头(§5.2) | 必含 |
...
**sketch 数据契约(`#bd-sketch-data`)**:一段 JSON = `napkin.sketch = {type, caption, nodes[], edges[]}`(schema 见 method §3 餐巾纸压缩)。`type∈{cascade,fork,loop}`;`nodes[]` 6-12 个 `{id, label, note?, mid?}`(`mid:true` = 中间产物,green 实底白字);`edges[]` `{from, to, label}`。脚本层 `initSketch()` 读它 → **横向分层布局**(按边方向做最长路径分层,层=列、x 递增,源节点在左、逐层向右,列内纵向堆叠居中)→ 生成内联 `<svg>` 挂到 `#bd-sketch`。**nodeData 之外零硬编码,换书换数据即出图(通用生成器)**。

**渲染规格(直角经典 · 浅色 · 静态一屏)**:

- **节点**:圆角矩形 `.sk-node`(`fill:var(--surface-strong)`、`stroke:var(--green)`)+ 居中文字 `.sk-label`(`fill:var(--ink)`);中间产物与终点节点加类 `.sk-mid` / `.sk-end`(二阶实底:`fill:var(--green)`,文字 `.sk-tmid` / `.sk-tend` 用 `--white`)。节点宽随 label 字数自适应,保证文字不溢出。
- **引线**:父→子 **直角肘形折线** `.sk-line`(`fill:none;stroke:var(--green);shape-rendering:crispEdges`)-- **禁贝塞尔曲线**;源节点**右缘**出横段 → 竖段 → 横段 → 落到目标节点**左缘**(横向布局,层=列、x 递增),汇流(多入一)时多条肘线收进同一顶点。小箭头 `.sk-arrow`(`fill:var(--green)`)标**向右**因果方向。
...
| 4 | **读完自检** `<div class="questions">` | distill:`self_check[]`(`q` / `followup` / `anchor`,4-8 条) | 整组收尾不打散。板头一句「这几个问题,建议每隔一段时间回来问自己一遍」。每条 `<article class="question" data-source>`=大号编号 + `<h3>`=`q`(第二人称主问句,含「你」)+ `<p>`=`followup`(**直接显示、不藏**)+「回到第 N 章 →」回链(`<a href="#ch-N">`,循环落点)。**无勾选、无打分、无记录**(决策 D11) | 必含 |
...
|---|---|---|---|---|
| **作者页** | `<section class="subpage" id="sub-author" hidden>` | enrich:`author_page`(结构化) | 见 §1.3.1(infobox 归一栏 + 五栏,`au-*` class 契约见本节与 author-page-skeleton.html) | `author_page===null` → 子视图 + 全部入口(刊头 `.cb-author-link` + ⑤作者书架栏头链)一并删 |
...
#### 1.3.1 作者页内部规范(F2,`au-*` class 契约同 author-page-skeleton.html,类名前缀 `au-`)
...
- **删除动作**:该区块 `<!-- SLOT:XXX -->` 到 `<!-- /SLOT:XXX -->` 连同注释对整段删;删子视图连同其入口(刊头 `.cb-author-link` 外壳→纯文本、⑤书架栏头链、③观点入口卡)。
...
| **T5** | **论点式标题** | `chapters[].title` / `.bd-chapter>h3` / `.chapter-toc` 章行 / 脑图二级节点文字 禁零信息标题,命中任一返工:① 黑名单正则 `^(第?\d+[章节讲集]?\|视频\d+)$`(以 method §7.5 G8 为准);② 有效长度 <8 字;③ 落通用容器词表(`章节脉络/全书脉络/内容概要/核心内容/主要观点/金句墙/金句/总结/概述/前言/结语`)。判断句语义靠蒸馏自查,verify 机拦 ①②③ |
...
### 2.3 视觉规格(banner 纸底绿带 / 6 级字阶 / 返回胶囊,数值以本节正文 + brand-tokens.md token 表为准,零 hex)
...
- **`nodeData`**(双层知识树,单根):`{ id, topic, root?, children?[], tags?[], hyperLink?, expanded?, _src? }`
  ...
  - **三级 = 支撑**(每个二级 ≤4 个,只收三类,排序固定 🧠→💡→⚖️):🧠 机制/心智模型、💡 核心概念、⚖️ 决策规则(当X→做Y)。**动宾/名词短语 ≤14 字,禁冒号双段式**。数据中的 emoji 前缀用于机器识别；渲染时必须展开成 `🧠 原理｜…`、`💡 概念｜…`、`⚖️ 规则｜…`，并在画布显示完整图例“🧠 原理/模型、💡 核心概念、⚖️ 行动规则”。emoji 只作辅助编码，不得单独承担类别语义。
...
- **三级类别文字始终可见**:节点使用“emoji + 短类别词 + 具体内容”，画布左上提供完整图例；颜色和 emoji 都只是冗余提示，去掉任一仍能读懂类别。
...
- **verify 门禁(自绘 SVG 版)**:静态 `lint_mindmap` 审 nodeData 树 —— ① 一级分支挂 hyperLink 叶子;② `#ch-N` 不死链;③ 二级缺 `tags[0]` 或 `tags[0]` 有效长 <8 字拦截;④ 全图 topic 总字数 >900 拦截(arrows/summaries 相关门禁随字段作废一并删除)。**SVG 结构断言**(脑图区真出 `<svg>`、文字节点数达标、**无关联箭头元素**、`window.__mm` viewer 已跑、`mm-stage` 走 viewBox 非 transform、点二级真跳章)由 Playwright 冒烟 `smoke()` 断。per-node 分级字数(≤10/≤14/≤12)靠蒸馏自查,不进硬门禁。
...
- **超预算合法裁法**:① 删净 dummy / 未用槽位;② 先压 excerpts,再压封面图(WebP data URI / 降尺寸),再收紧啰嗦 lede / section-lead;③ 仍超再报用户。**禁**删骨架交互脚本(§6.1,无 vendor)/ 删必需区块 / 删 `data-source` / 压 narrative 跌破 G9。
