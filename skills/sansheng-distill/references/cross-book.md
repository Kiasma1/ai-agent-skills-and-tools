...
> **执行手册,不是理论文**。管线跑到 Step4 时 agent 逐节照做:把本书 `distill.json.concepts[]` 按 **5-tag 增量合并协议**登记进全局跨书索引,并为页面板块⑤ `.crossbook` 跨书回声生成已蒸书互链。
...
| `entry.stance` | entry | 强烈建议(本书对该概念的立场,供 ⑤ 跨书回声分歧展示) |
| `entry.quote` | entry | 强烈建议(一句原文,供 ⑤ 跨书回声分歧呈现引证) |
...
| **CONTRADICTS** | **立场相反**;**禁止抹平**,不揉成「都对/要平衡」,⑤ `.crossbook` 分歧行左右并列原样呈现 | 某成长投资著作主张「为伟大公司支付溢价、放弃安全边际」,与格雷厄姆低价买入直接对立 → CONTRADICTS;两方立场分别照录,不合并成一句 |
...
> **CONTRADICTS 硬约束**:分歧是跨书阅读的价值来源,不是要消除的噪声。既不改写本书立场去迁就索引,也不改写索引旧条目去迁就本书;两条 entry 都留在同一 `concept` 下,⑤ `.crossbook` 分歧行原样并列呈现(§5)。
...
## 5. 页面互链规则(⑤ `.crossbook` 跨书回声渲染)

Step4 合并后,为页面 ⑤ `.crossbook` 跨书回声的「跨书观点」子块生成**已蒸书互链**:凡本书某概念在索引里也出现于**其他已蒸书**(entry.book_slug ≠ 本书 slug),就为每本这样的书渲染一条链接:
...
- `#ch-{N}` = 锚点:对方页 `.bd-chapter` 的 id 是 `ch-{chapters[].no}`(page-skeleton 章节 id 契约)。
...
- **CONTRADICTS 的两方并排**:同一概念下若有 CONTRADICTS 关系的 entry,⑤ `.crossbook` 用**分歧行**(`.xb-echo`)并列本书与对方的 `stance`/`quote`,原样呈现分歧,不给「谁对」结论(呼应 §3 CONTRADICTS 硬约束)。
...
- **跨书互链**:⑤ `.crossbook` 的兄弟 slug 链接在单文件 HTML 里是相对路径;若托管到站点,由站点路由把兄弟 slug 解析成站内可达路径。

---

## 6. 索引没有的概念 → 转 enrich 联网(衔接 enrich.md)

§2 ② 里判为 **NEW_CONCEPT / 未命中索引**的概念,若还想在 ⑤ `.crossbook` 展示「这个概念其他(未蒸)书怎么看」:

- **不**在本步 register(它只登记本书自己的 entry)。
- 转 `enrich.md` §6 的 `cross_book_external`:联网找该概念在外部书里的立场,每条带 `source` URL,写进 `enrich.json`。
- 于是 ⑤ `.crossbook` 的「跨书观点」= Step4 互链(已蒸书,§5)+ enrich 外部条目(未蒸书,带 URL)两部分并列;两边共用 §2 ① 的同一次 query 结果切分,互不重复(命中索引走互链,未命中走外部联网)。
...
- [ ] CONTRADICTS 分歧原样保留,未抹平;⑤ `.crossbook` 分歧左右并列(§3/§5)。
- [ ] ⑤ `.crossbook` 互链写相对路径 `{对方slug}.html#ch-{N}`,无绝对/http 路径;standalone 不跳达属预期(§5)。
