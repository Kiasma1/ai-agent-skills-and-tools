...
> **v3 变化**:联网增补落成 **2 张页内全屏子视图** + 板块⑤内联(satellite_form=B,仍单文件填充)-- `author_page`(作者页)/ `views_page`(观点对照页)做子视图;`similar_page`(同类书)**不再做独立子视图,内联板块⑤**(同类书卡 + 阅读路径);外加两个内联模块 `reviews`(书评,内联③)/ `cross_book_external`(跨书外部立场,内联⑤)沿用。
...
| 键 | 页面呈现 | 定义 | 降级写法 |
|---|---|---|---|
| `author_page` | **子视图 `#sub-author`**(全屏,infobox 归一栏 + 五栏)+ **刊头作者名胶囊链 `.cb-author-link`** | 结构化人物档案:infobox 基础信息 + 人物经历/成就影响/争议评价/他与这本书/主要作品五栏;刊头作者名即入口,点开看全档 | 整块置 `null`(胶囊链 + 入口一并隐藏) |
| `similar_page` | **⑤ 同类书卡 + 阅读路径**(内联;`#sub-similar` 子视图 v3 已撤销) | 同类书推荐(带关系/适合谁/先后读)+ 一条阅读路线 | 整块置 `null` 或 `items: []` |
| `views_page` | **子视图 `#sub-views`**(全屏) | 书中核心观点的国内外赞同方/质疑方对照,每条带来源 | 整块置 `null`;某 topic 缺一方 → 该方 `[]` |
| `reviews` | **内联 ③ 批判与评价·别人怎么评**(评分 `rating` 另喂 ③ 裁决条) | 评分 + 代表性书评(正反都收),每条带来源 | 整块置 `null`;有评分无书评 → `items: []` 保留 `rating` |
| `cross_book_external` | **内联 ⑤**(跨书表·外部书立场) | 同一概念在**未蒸过的**其他书里的立场 | 整块置 `null` 或 `[]`(见 §6) |

**作者短卡文案 = `author_page.tagline`**(≤50 字,双用途:作者页导语 + ⑤ 作者书架/入口短卡 desc,渲染层直接取用,比截断 bio_long 更短更钉人);刊头作者名胶囊链点开 `#sub-author` 看完整结构化档案。因此 enrich.json **不再有独立 `author_card` 字段**--短卡是 `author_page` 的派生视图,单一来源,避免两处数据漂移。旧版 `similar_books` 已由 `similar_page` 取代(带 author/why/fit/order/reading_path,内容内联板块⑤),不再单列。
...
- `author_page === null` → 子页 `#sub-author` + 全部入口(刊头作者名胶囊链 `.cb-author-link` + ⑤ 作者书架栏头链)一并删(沿用 v2)。
...
**v2 预算加大**:三个二级页各起一轮**专门**搜索(不再是 v1 单模块一击即止);当前 Agent 支持任务委派时可按块派高推理能力 worker 并行,否则主 Agent 串行。每块按下方降级链从左到右尝试,**前一档拿到可用信息即停**;某档抓取失败(含反爬,见 §4)即降级下一档;全部档位都拿不到 → 该块整块置 `null`。
...
| 观点对照页(views_page) | **国内轨 + 国外轨两轨分搜**(见 §3.3,不是单链降级) |
...
按 `agent-compat.md` 选择:专用搜索连接器/MCP → 当前 Agent 内置网页搜索 → 浏览器抓取。取具体页面正文用当前 Agent 的页面读取或浏览器能力;某一提供器报限流/质量差即降级下一个,不在同一提供器上反复重试。
...
| `views_page` | 国内外观点对照 | **评论区实质观点 + 外部看法**:`topics`=本系列几个核心主张;`supporters`/`critics` 来自 ① `comments.json` 里有实质论点的评论(照录)+ ② 联网找的外部对该主题的不同看法;`source`=评论所在**视频 URL** 或外部 URL | comments.json + 联网 |
...
- `views_page` = **深度观点对照**:先定 2-5 个本系列核心主张(`topics[].viewpoint`),对每个主张:① 从 comments.json 里挑**有实质论点**的赞同/质疑评论(照录,source=该评论所在视频 URL);② 联网找外部(其他博主/媒体/文章)对该主题的不同看法(source=外部 URL)。组织成子视图。
